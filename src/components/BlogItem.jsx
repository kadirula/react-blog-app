            ...
            {blogs.map((blog, index) => (
                <div key={index} className='blog mx-3 mb-5 shadow-xl rounded-lg overflow-hidden relative'>
                    <div className='overflow-hidden max-h-64 relative'>
                        <img src={blog.image} className='hover:scale-105 transition-all w-full ' alt="" />
                        <Link to='/author-blog' className='absolute top-0 right-0 bg-indigo-500 text-white px-5 font-bold py-1 z-10'>{blog.author}</Link>
                        <Link to='/category-blog' className='absolute bottom-0 left-0 bg-indigo-500 text-white px-5 font-bold py-1 z-10'>{blog.category}</Link>
                    </div>
